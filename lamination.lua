      ...
      if Pages[3].lamination[Pages[3].position] ~= 0 then
        ...
        if (counter - 1) % div + 1 == 1 then
          ...
          if octave ~= 0 then
            ...
            local note_num = Pages[1].lamination[Pages[1].position]
            if note_num ~= 0 and Running then
              local pitch = Scale[note_num]
              if params:get("out") == 1 or params:get("out") == 3 then
                engine.hz(MusicUtil.note_num_to_freq(pitch + 12 * octave))
